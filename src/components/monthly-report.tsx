...
export default function MonthlyReport() {
  ...
  // కస్టమ్ పరిధి: Previous Month స్టార్ట్ డేట్ & Current Month ఎండ్ డేట్
  const [startCutoffDay, setStartCutoffDay] = useState<string>('1'); // 1 = 1st of month, 16, 7, 26 etc.
  const [endCutoffDay, setEndCutoffDay] = useState<string>('end');   // 'end' = last day, 15, 6, 25 etc.
  ...
  // ఎంచుకున్న మునుపటి మరియు ప్రస్తుత నెల తేదీల ఆధారంగా డైనమిక్ పరిధి లెక్కింపు
  const daysInCycle = useMemo(() => {
    // సాధారణ 1 నుండి నెలాఖరు వరకు అయితే
  ...
  // 1. నెలవారీ లెక్కింపు (Single-Status Strict Rule)
  ...
  // 2. వార్షిక లెక్కింపు - ఖచ్చితమైన 1:1 సింగిల్ కౌంట్ (డబుల్ P సమస్య తొలగించబడింది)
  const yearlyReportData = useMemo(() => {
    ...
    return employees.filter(e => e.status === 'Approved' || e.status === 'Resigned').map(employee => {
      ...
      daysInYear.forEach(day => {
        ...
        // కచ్చితమైన సింగిల్ కౌంట్ - ఒక రోజుకు కేవలం ఒకే కేటగిరీ
        if (rawBenefit === 'C-off Earned') {
          totalCoffsEarned += (rawExtra > 0 ? Number((rawExtra / 8).toFixed(2)) : 1);
        } else if (rawBenefit === 'C-off Used' || status === 'C-off') {
          totalCoffsUsed += (rawExtra > 0 ? Number((rawExtra / 8).toFixed(2)) : 1);
        } else if (status === 'Present') {
          pCount++; // కేవలం ప్రెజెంట్ అయితేనే 1 సారి పెరుగుతుంది
        } else if (status === 'OT') {
          pCount++; // OT డ్యూటీ రోజు కూడా 1 సారి మాత్రమే పెరుగుతుంది
      ...
      return {
        id: employee.id,
        name: employee.name,
        p: pCount, // కచ్చితమైన సింగిల్ ప్రెజెంట్ కౌంట్
      ...
      <Card className="border border-border/80 shadow-md">
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0 px-4 sm:px-6">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            ...
            {/* === కస్టమ్ పరిధి: PREVIOUS MONTH & CURRENT MONTH 2 BOXES === */}
            {viewMode === 'month' && (

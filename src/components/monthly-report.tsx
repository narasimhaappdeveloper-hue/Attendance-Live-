...
export default function MonthlyReport() {
  ...
  // 1. నెలవారీ లెక్కింపు (OT & Paid Days డబుల్ కౌంట్ బగ్స్ సరిచేయబడ్డాయి)
    ...
    return employees.filter(e => {
      ...
      const dailyStatus = daysInCycle.map(day => {
        ...
        if (rawBenefit === 'C-off Earned') {
          ...
          countP++; // ఆ రోజు డ్యూటీ చేశారు కాబట్టి 1 Paid Day
        } 
        // C-off Used
        ...
        // Present లేదా OT Duty (ఖచ్చితంగా ఒక్క ప్రెజెంట్ మాత్రమే)
        else if (status === 'Present' || status === 'OT') {
          displayBadge = status === 'OT' ? 'OT' : 'P';
          countP++; // కేవలం ఒకే ఒకసారి పెరుగుతుంది (డబుల్ అవ్వదు)
          
          if (rawBenefit === 'OT' || status === 'OT') {
            finalOT = rawExtraHours;
            totalOTHours += finalOT; // OT కేవలం ఒకేసారి మాత్రమే కలుస్తుంది!
          }
        } 
      ...
      // ఖచ్చితమైన Paid Days లెక్కింపు: countP + W + H + C-off Used + (HD * 0.5)
      const presentsCount = countP;
  ...
  // 2. వార్షిక లెక్కింపు (Single Count Strict Rule)
    ...
    return employees.filter(e => e.status === 'Approved' || e.status === 'Resigned').map(employee => {
      ...
      daysInYear.forEach(day => {
        ...
        if (rawBenefit === 'C-off Earned') {
          ...
          pCount++; // డ్యూటీ చేశారు కాబట్టి 1 Present
        } else if (rawBenefit === 'C-off Used' || status === 'C-off') {
          totalCoffsUsed += (rawExtra > 0 ? Number((rawExtra / 8).toFixed(2)) : 1);
        } else if (status === 'Present' || status === 'OT') {
          pCount++; // కేవలం ఒకే ఒకసారి మాత్రమే పెరుగుతుంది
          if (rawBenefit === 'OT' || status === 'OT') {

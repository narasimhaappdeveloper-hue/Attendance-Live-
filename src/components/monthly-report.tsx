...
export default function SalarySlips() {
  ...
  const calculatedData = useMemo(() => {
    ...
    return employees.filter(e => {
      ...
      daysInMonth.forEach(day => {
        ...
        // 1. P కేవలం ప్రెసెంట్ అయినప్పుడు మాత్రమే పెరుగుతుంది
      ...
      // ==========================================
      // కొత్త నియమాలు: P కేవలం P గానే ఉంటుంది
      // Paid Days లో మాత్రం: P + W + H + OT + (HalfDay * 0.5)
      // ==========================================
      const exactPresentDays = p; 
      const otDaysEquivalent = ot > 0 ? (ot / 8) : 0; // 8 గంటల OT = 1 పెయిడ్ డే

      // పెయిడ్ డేస్ లెక్కింపు
      const totalPaidDays = Number((exactPresentDays + w + h + otDaysEquivalent + (hd * 0.5)).toFixed(2));
      const lopDays = Math.max(0, totalDaysCount - (exactPresentDays + w + h + (hd * 0.5)));

      // బేసిక్ పే మరియు రేట్లు
      const basePay = (employee as any).basicSalary || ((employee.dailyRate || 500) * 30);
      const dailyRate = basePay / 30; // Basic / 30[cite: 1]
      const hourlyRate = dailyRate / 8; // (Basic / 30) / 8[cite: 1]

      // పెయిడ్ డేస్ ఆధారంగా వచ్చే ఆర్జిత బేసిక్ శాలరీ
      const earnedBasic = Math.round(totalPaidDays * dailyRate);

      // అలవెన్సులు
      const daAmount = Math.round((basePay * ((employee as any).daPercent || 0)) / 100); //[cite: 1]
      const hraAmount = employee.hra || Math.round((basePay * ((employee as any).hraPercent || 0)) / 100); //[cite: 1]
      const conveyance = employee.conveyance || 0; //[cite: 1]
      const special = employee.specialAllowance || 0; //[cite: 1]
      ...
      const earnings = {
        ...
        otPay: Math.round(otDaysEquivalent * dailyRate), // OT పెయిడ్ డేస్ అమౌంట్
      ...
      const autoPF = employee.isPFEnabled ? Math.round(earnedBasic * 0.12) : 0; // Basic * 12%[cite: 1]
      const autoESI = (employee.isESIEnabled && grossEarnings <= 21000) 
        ? Math.round(grossEarnings * 0.0075) // Gross * 0.75%[cite: 1]
        : 0;

      let autoPT = 0;
      if (employee.isPTEnabled) {
        if (grossEarnings > 20000) autoPT = 200; //[cite: 1]
        else if (grossEarnings > 15000) autoPT = 150; //[cite: 1]
      }
      ...
      // ఫైనల్ నెట్ శాలరీ (Gross - Deductions)[cite: 1]
      ...
      return {
        employee,
        stats: { p: exactPresentDays, l, h, c, w, ot, a, hd, totalPaidDays, lopDays, totalLateHoursCut, otDaysEquivalent },
        earnings,
        deductions,
        grossEarnings,
        totalDeductions,
  ...
  const getWhatsAppMessage = (item: (typeof calculatedData)[0]) => {
    ...
    return `*OFFICIAL SALARY SLIP - ${monthName.toUpperCase()}*\n` +
      ...
      `📅 *Present (P):* ${item.stats.p} Days\n` +
      `🏖️ *Week-Off (W):* ${item.stats.w} Days\n` +
      `🎉 *Holiday (H):* ${item.stats.h} Days\n` +
      `⏱️ *OT Equivalent:* ${item.stats.otDaysEquivalent.toFixed(1)} Days (${item.stats.ot} hrs)\n` +
      `✅ *TOTAL PAID DAYS:* ${item.stats.totalPaidDays} Days\n` +
      `--------------------------------\n` +
      `💰 *Basic Pay (from Paid Days):* ₹${Math.round(item.earnings.basic).toLocaleString('en-IN')}\n` +
      `➕ *Gross Pay:* ₹${Math.round(item.grossEarnings).toLocaleString('en-IN')}\n` +
  ...
  return (
    <div className="space-y-6">
         ...
         <div className="flex items-start gap-3">
            ...
            <div>
               <p className="font-bold flex items-center gap-2">
                 Attendance & Payroll Strict Rules Active:
               ...
               <p className="text-xs text-muted-foreground mt-0.5">
                  <b>P కాలమ్:</b> కేవలం ప్రెసెంట్ రోజులు మాత్రమే లెక్కిస్తుంది. | <b>Paid Days:</b> P + W + H + OT Days తో లెక్కించబడి, దాని ఆధారంగా Net Salary తయారవుతుంది.
               </p>
            </div>
         </div>
      ...
      <Card>
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0">
          <div>
            ...
            <CardDescription>Strict P count & Paid-days calculated salary disbursement.</CardDescription>
          </div>
            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow>
                  ...
                  <TableHead className="whitespace-nowrap text-blue-600 font-bold">Present (P only)</TableHead>
                  <TableHead className="whitespace-nowrap">W / H / OT (Days)</TableHead>
                  <TableHead className="whitespace-nowrap font-bold text-green-700">Total Paid Days</TableHead>
                  <TableHead className="whitespace-nowrap">Gross Pay</TableHead>
                  <TableHead className="whitespace-nowrap">Deductions</TableHead>
                  <TableHead className="whitespace-nowrap font-black">Net Salary</TableHead>
                  <TableHead className="text-right whitespace-nowrap">Dispatch</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {calculatedData.map((item, idx) => (
                  <TableRow key={idx}>
                    ...
                    {/* కేవలం వాస్తవ Present (P) మాత్రమే చూపించే కాలమ్ */}
                    <TableCell className="text-sm font-bold text-blue-600">{item.stats.p} P</TableCell>
                    <TableCell className="text-xs text-muted-foreground">
                      {item.stats.w}W + {item.stats.h}H + {item.stats.otDaysEquivalent.toFixed(1)}OT
                    </TableCell>
                    {/* P + W + H + OT తో కలిసిన Paid Days */}
                    <TableCell className="text-sm font-black text-green-700">{item.stats.totalPaidDays} Days</TableCell>
                    <TableCell className="font-semibold text-sm">₹{Math.round(item.grossEarnings).toLocaleString()}</TableCell>
                    <TableCell className="text-destructive text-sm">₹{Math.round(item.totalDeductions).toLocaleString()}</TableCell>
                    {/* Paid Days ఆధారంగా వచ్చిన నెట్ శాలరీ */}
                    <TableCell className="font-bold text-primary text-sm bg-primary/5">₹{Math.round(item.netPay).toLocaleString()}</TableCell>
      ...
      {/* పే-స్లిప్ ప్రివ్యూ డైలాగ్ బాక్స్ */}
      <Dialog open={!!selectedSlip} onOpenChange={() => setSelectedSlip(null)}>
        <DialogContent className="max-w-4xl p-0 overflow-hidden border-none shadow-2xl h-[90vh] sm:h-auto overflow-y-auto">
          ...
          <DialogDescription className="sr-only">Detailed salary slip based on strict attendance and paid days calculation.</DialogDescription>
          {selectedSlip && (
            <div className="bg-white text-slate-900 printable-area text-[11px] sm:text-[13px] font-sans min-h-full">
              ...
              <div className="p-4 sm:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 bg-slate-50/50">
                <div className="space-y-2 sm:space-y-4">
                  ...
                  <div className="grid grid-cols-2">
                    <span className="text-slate-400 font-bold uppercase text-[8px] sm:text-[9px]">Present Days (P Only)</span>
                    <span className="font-bold text-blue-600">: {selectedSlip.daysPresent} Days</span>
                  </div>
                </div>
                <div className="space-y-2 sm:space-y-4">
                  <div className="grid grid-cols-2">
                    <span className="text-slate-400 font-bold uppercase text-[8px] sm:text-[9px]">Total Paid Days (P+W+H+OT)</span>
                    <span className="font-black text-green-700">: {selectedSlip.daysPaid} Days</span>
                  </div>
                  <div className="grid grid-cols-2">
                    <span className="text-slate-400 font-bold uppercase text-[8px] sm:text-[9px]">OT Hours Logged</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 border-y">
                <div className="border-r-0 sm:border-r border-b sm:border-b-0 p-3 sm:p-4 space-y-1.5">
                   <div className="font-bold uppercase text-[10px] text-muted-foreground border-b pb-1">Earnings (Paid Days ఆధారంగా)</div>
                   <div className="flex justify-between"><span>Basic Pay (Total Paid Days × Daily Rate)</span><span className="font-bold">₹{Math.round(selectedSlip.earnings.basic).toLocaleString()}</span></div>
                   ...
                   <div className="flex justify-between"><span>DA</span><span className="font-bold">₹{Math.round(selectedSlip.earnings.da || 0).toLocaleString()}</span></div>
                ...
                <div className="p-3 sm:p-4 space-y-1.5">
                   ...
                   <div className="flex justify-between text-destructive"><span>LOP (Absent/Late)</span><span className="font-bold">₹{Math.round(selectedSlip.deductions.lop).toLocaleString()}</span></div>
              ...
              <div className="p-6 sm:p-8 flex items-center justify-between bg-primary/5">
                <div>
                  <h3 className="text-xs sm:text-sm font-black text-primary uppercase">Net Salary Payable (Gross - Deductions)</h3>
                  <p className="text-xs text-muted-foreground">Calculated strictly on Paid Days: {selectedSlip.daysPaid}</p>
                </div>

...
export default function MonthlyReport() {
  ...
  // 1. నెలవారీ లెక్కింపు (P & C-off ఖచ్చితమైన గణన)
  const reportData = useMemo(() => {
    return employees.filter(e => {
      ...
      // ఖచ్చితమైన కౌంట్స్ (డబుల్ కౌంట్ లేకుండా)
      const stats = dailyStatus.reduce((acc, curr) => {
        if (curr.status === 'Present') acc.Present += 1;
        else if (curr.status === 'Absent') acc.Absent += 1;
        else if (curr.status === 'Week-off') acc['Week-off'] += 1;
        else if (curr.status === 'Leave') acc.Leave += 1;
        else if (curr.status === 'Holiday') acc.Holiday += 1;
        else if (curr.status === 'Half-Day') acc['Half-Day'] += 1;
        else if (curr.status === 'OT') acc.OT += 1;
        else if (curr.status === 'C-off') acc['C-off'] += 1;

        acc.totalOT += curr.ot;
        return acc;
      }, { Present: 0, Absent: 0, 'Week-off': 0, Leave: 0, Holiday: 0, 'C-off': 0, 'Half-Day': 0, OT: 0, totalOT: 0 });

      // డబుల్ లెక్కింపును సరిచేసాం: P కేవలం Present + OT మాత్రమే
      const presentsCount = stats.Present + stats.OT;
  ...
  // 2. పూర్తి సంవత్సరానికి లెక్కింపు
  ...
  // మంత్లీ గ్రాండ్ టోటల్స్ లో కూడా డబుల్ కౌంట్ సరిచేసాం
  const grandTotals = useMemo(() => {
    return reportData.reduce((acc, curr) => {
      acc.totalDays += curr.totalDays;
      acc.present += curr.stats.Present; // డబుల్ యాడ్ కాకుండా కేవలం stats.Present
  ...
  const saveDayStatus = () => {
    ...
    // C-off Earned అయితే
    else if (editForm.extraShiftBenefit === 'C-off Earned') {
    ...
    markExtraStatus(
      ...
      editForm.extraShiftBenefit as any // C-off Earned లేదా C-off Used ఖచ్చితంగా స్టోర్ అవుతుంది
    );
    setEditingDay(null);
  };

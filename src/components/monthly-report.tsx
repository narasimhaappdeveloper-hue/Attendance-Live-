...
// C-off స్టేటస్‌ను సరిగ్గా గుర్తించే హెల్పర్ ఫంక్షన్
const isCOffStatus = (st?: string | null) => {
  if (!st) return false;
  const s = st.toLowerCase().trim();
  return s === 'c-off' || s === 'coff' || s === 'c_off' || s === 'comp-off';
};
...
export default function MonthlyReport() {
  ...
  const reportData = useMemo(() => {
    return employees.filter(e => {
      ...
      const dailyStatus = daysInMonth.map(day => {
        ...
        // C-off క్రెడిట్ సంపాదించడం (8 గంటలకు 1 C-off)
        if (assignedBenefit === 'C-off') {
          if (rawExtraHours > 0) {
            cOffCredit = Number((rawExtraHours / 8).toFixed(2));
      ...
      const coffCount = stats['C-off'] || 0; // C-off ని కూడా పెయిడ్ డే గానే లెక్కిస్తాం
  ...
  // 2. ఎంచుకున్న పూర్తి సంవత్సరానికి పక్కా C-off ట్రాకింగ్
  const yearlyReportData = useMemo(() => {
    ...
    return employees.filter(e => e.status === 'Approved' || e.status === 'Resigned').map(employee => {
      ...
      daysInYear.forEach(day => {
        ...
        const benefit = extra?.extraShiftBenefit || 'OT';
        if (benefit === 'C-off') {
          if (rawExtra > 0) totalCoffsEarned += Number((rawExtra / 8).toFixed(2));
        } else {
        ...
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0 px-4 sm:px-6">
          <div className="w-full">
            ...
            <CardDescription className="text-xs sm:text-sm mt-1">
              {viewMode === 'month' 
                ? '👉 1 నుండి 30/31 వరకు తేదీలను చూడండి లేదా పక్కన ఉన్న "Full Year Summary" బాక్స్ నొక్కి సంవత్సరం మొత్తం C-off లెక్కలు చూడండి.' 
                : `👉 ${selectedYear} సంవత్సరానికి సంబంధించిన మొత్తం P, OT, A, W, H మరియు C-off పెండింగ్/యూజ్డ్ లెక్కలు.`}
            </CardDescription>
          </div>
              ...
              <div className="mx-2 sm:mx-0 p-4 bg-gradient-to-r from-teal-50 to-blue-50 border border-teal-200 rounded-xl flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-teal-800">C-off Earned (సంపాదించినవి) - C-off Used (వాడినవి) = C-off Pending (మిగిలినవి)</p>
                  </div>
                </div>
                ...
                >
                  <thead>
                    <tr style={{ backgroundColor: '#f1f5f9', height: '48px', borderBottom: '2px solid #cbd5e1' }}>
                      ...
                      <th style={{ width: '100px', fontWeight: 'bold', color: '#0f766e', backgroundColor: '#ccfbf1', borderRight: '1px solid #cbd5e1' }}>C-off Earned</th>
                      <th style={{ width: '90px', fontWeight: 'bold', color: '#6b21a8', backgroundColor: '#fae8ff', borderRight: '1px solid #cbd5e1' }}>C-off Used</th>
                ...
                <tbody>
                  {reportData.map(row => (
                    <tr key={row.id} style={{ borderBottom: '1px solid #e2e8f0', height: '48px' }}>
                      ...
                      {/* C-off ఖచ్చితమైన లెక్కింపు: ఎన్ని వాడారు / ఎన్ని సంపాదించారు */}

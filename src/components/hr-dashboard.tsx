    ...
    <div className="space-y-6">
      ...
      <Card className="border border-border/80 shadow-md">
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0 px-4 sm:px-6">
          <div className="w-full">
            ...
            <CardDescription className="text-xs sm:text-sm mt-1">
              👉 1 నుండి 30/31 వరకు అన్ని తేదీలు చూడటానికి టేబుల్‌ను ఎడమవైపుకి జరపండి (Scroll Right).
            </CardDescription>
          </div>
        ...
        <CardContent className="p-0 sm:p-4">
          {/* పక్కా హారిజాంటల్ స్క్రోల్ కంటైనర్ */}
          <div 
            style={{
              width: '100%',
              overflowX: 'scroll',
            ...
            className="border-t sm:border rounded-none sm:rounded-xl"
          >
            {/* కచ్చితమైన వెడల్పు (2100px) తో నేరుగా ప్రామాణిక టేబుల్ */}
            <table 
              style={{
                width: '2100px',
                minWidth: '2100px',
            ...
            >
              <thead>
                <tr style={{ background: '#f1f5f9', height: '48px', borderBottom: '2px solid #cbd5e1' }}>
                  <th style={{ width: '170px', padding: '8px 12px', textAlign: 'left', fontWeight: 'bold', fontSize: '13px', borderRight: '1px solid #cbd5e1' }}>
                    Employee Name
                  </th>
                    ...
                    return (
                      <th 
                        ...
                        style={{
                          ...
                          backgroundColor: isSun ? '#fee2e2' : isSat ? '#fef3c7' : 'inherit',
                          color: isSun ? '#b91c1c' : isSat ? '#b45309' : '#334155'
                        }}
                      >
                        <div style={{ fontSize: '12px', fontWeight: 'bold', lineHeight: '1.2' }}>{format(day, 'd')}</div>
                        <div style={{ fontSize: '9px', textTransform: 'uppercase', opacity: 0.8 }}>{format(day, 'EEE')}</div>
                      </th>
                    );
                  })}

                  <th style={{ width: '50px', textAlign: 'center', fontWeight: 'bold', color: '#15803d', background: '#dcfce7', borderRight: '1px solid #cbd5e1' }}>P</th>
                  <th style={{ width: '50px', textAlign: 'center', fontWeight: 'bold', color: '#b91c1c', background: '#fee2e2', borderRight: '1px solid #cbd5e1' }}>A</th>
                  <th style={{ width: '55px', textAlign: 'center', fontWeight: 'bold', color: '#0f766e', background: '#ccfbf1', borderRight: '1px solid #cbd5e1' }}>C-off</th>
                  <th style={{ width: '55px', textAlign: 'center', fontWeight: 'bold', color: '#dc2626', background: '#f8fafc', borderRight: '1px solid #cbd5e1' }}>Late</th>
                  <th style={{ width: '60px', textAlign: 'center', fontWeight: 'bold', color: '#1d4ed8', background: '#eff6ff', borderRight: '1px solid #cbd5e1' }}>Paid</th>
                  <th style={{ width: '110px', textAlign: 'center', fontWeight: 'bold', color: '#1e3a8a', background: '#dbeafe' }}>Net Sal</th>
                </tr>
              </thead>
              ...
              <tbody>
                {reportData.map(row => (
                  <tr key={row.id} style={{ borderBottom: '1px solid #e2e8f0', height: '48px' }}>
                    <td style={{ width: '170px', padding: '8px 12px', borderRight: '1px solid #cbd5e1', background: '#fff' }}>
                      <div style={{ fontWeight: 'bold', fontSize: '13px', color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {row.name}
                      </div>
                      ...
                      return (
                        <td 
                          ...
                          style={{
                            ...
                            cursor: 'pointer',
                            backgroundColor: isSun ? 'rgba(254, 226, 226, 0.3)' : isSat ? 'rgba(254, 243, 199, 0.3)' : '#fff'
                          }}
                        >
                          <div 
                            style={{
                    ...
                    <td style={{ width: '50px', textAlign: 'center', fontWeight: 'bold', color: '#15803d', borderRight: '1px solid #cbd5e1' }}>{row.stats.Present || 0}</td>
                    <td style={{ width: '50px', textAlign: 'center', fontWeight: 'bold', color: '#dc2626', borderRight: '1px solid #cbd5e1' }}>{row.stats.Absent || 0}</td>
                    <td style={{ width: '55px', textAlign: 'center', fontWeight: 'bold', color: '#0f766e', borderRight: '1px solid #cbd5e1' }}>{row.totalCoffsEarned.toFixed(1)}</td>
                    <td style={{ width: '55px', textAlign: 'center', fontWeight: 'bold', color: '#ef4444', borderRight: '1px solid #cbd5e1' }}>{row.totalLateHoursCut.toFixed(1)}</td>
                    <td style={{ width: '60px', textAlign: 'center', fontWeight: '900', color: '#1d4ed8', background: '#f8fafc', borderRight: '1px solid #cbd5e1' }}>{row.paidWorkingDays}</td>
                    <td style={{ width: '110px', textAlign: 'center', fontWeight: 'bold', color: '#1e3a8a', background: '#fff' }}>
              ...
              <tfoot>
                <tr style={{ background: '#f8fafc', height: '44px', fontWeight: 'bold', borderTop: '2px solid #cbd5e1' }}>
                  <td style={{ width: '170px', padding: '8px 12px', borderRight: '1px solid #cbd5e1', fontWeight: '900' }}>Grand Total</td>
                  
                  {daysInMonth.map((_, i) => (
                    <td key={`ft-date-${i}`} style={{ width: '46px', textAlign: 'center', color: '#94a3b8', borderRight: '1px solid #f1f5f9' }}>-</td>
                  ))}

                  <td style={{ width: '50px', textAlign: 'center', color: '#15803d', borderRight: '1px solid #cbd5e1', background: '#dcfce7' }}>{grandTotals.present}</td>
                  <td style={{ width: '50px', textAlign: 'center', color: '#b91c1c', borderRight: '1px solid #cbd5e1', background: '#fee2e2' }}>{grandTotals.absent}</td>
                  <td style={{ width: '55px', textAlign: 'center', color: '#0f766e', borderRight: '1px solid #cbd5e1', background: '#ccfbf1' }}>{grandTotals.coffsEarned.toFixed(1)}</td>
                  <td style={{ width: '55px', textAlign: 'center', color: '#dc2626', borderRight: '1px solid #cbd5e1' }}>{reportData.reduce((acc, r) => acc + r.totalLateHoursCut, 0).toFixed(1)}</td>
                  <td style={{ width: '60px', textAlign: 'center', color: '#1d4ed8', background: '#eff6ff', borderRight: '1px solid #cbd5e1' }}>{grandTotals.paidWorkingDays}</td>
                  <td style={{ width: '110px', textAlign: 'center', color: '#1e3a8a', background: '#dbeafe' }}>₹{Math.round(grandTotals.salary).toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
